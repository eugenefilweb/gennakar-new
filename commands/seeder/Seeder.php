...
{
    public abstract function attributes($key);
    ...
    public function summary()
    {
        ...
        if ($this->failed > 0) {
            ...
            foreach ($this->model_errors as $row => $validation) {
                $rows[] = [$row, json_encode($validation), json_encode($this->error_attributes[$row])];
            }

            if ($this->showProgress) {
                Console::output('Unsuccessfull inserts.');
                echo Table::widget([
                    'headers' => ['Row', 'Validation', 'Attributes'],
                    'rows' => $rows,
                ]);
            }
        }
    ...
    public function bulkSeed()
    {
        ...
        $rows = [];
        for ($key=1; $key <= $this->rows; $key++) { 
            $rows[] = $this->attributes($key);
        }

        if ($rows) {
            $arr = array_chunk($rows, 100);
            $columns = array_keys($rows[0]);
            foreach ($arr as $r) {

                $this->success += App::createCommand()
                    ->batchInsert($modelClass::tableName(), $columns, $r)
                    ->execute();
            }
        }
    ...
    public function seed()
    {
        ...
        for ($key=1; $key <= $this->rows; $key++) { 
            ...
            $model->load([App::className($model) => $this->attributes($key)]);

            if ($model->hasProperty('logAfterSave')) {
                $model->logAfterSave = false;
            }

            $newModel = $this->save($model, $key);

            if ($this->insert) {
                call_user_func($this->insert, $newModel, $key);
            }
        }
