    ...
    public function actionFindNameQrKeywords($keywords='', $suggestionOnly = 'true')
    {
        if ($suggestionOnly == 'true') {
            $models = Database::find()
                ->select(['id', 'CONCAT_WS(" ", `first_name`, `middle_name`, `last_name`) AS fullname'])
                ->where(['LIKE', 'CONCAT_WS(" ", `first_name`, `middle_name`, `last_name`)', $keywords])
                ->orderBy(['LENGTH(CONCAT_WS(" ", `first_name`, `middle_name`, `last_name`))' => SORT_ASC])
                ->asArray()
                ->limit(20)
                ->all();

            return $this->asJson(
                App::foreach($models, fn($model) => implode(' - ', [
                    $model['id'],
        ...
        try {
            list($id, $fullname) = explode(' - ', $keywords);

            if (($database = Database::findOne($id)) != null) {
                return $this->asJson([
                    'status' => 'success',
                    'database' => $database,
                    'html' => implode('<br>', [
                        ...
                        $this->renderAjax("view/{$database->viewTemplate}", [
                            'model' => $database
                        ])
                    ])
                ]);
