...
class Database extends ActiveRecord
{
    ...
    public function rules()
    {
        ...
        return $this->setRules([
            [['system_id', 'priority_sector', 'watershed_id'], 'integer'],
            [['age'], 'integer', 'max' => 999],

            [['last_name', 'first_name', 'sector_id', 'gender', 'barangay'], 'required', 'on' => $this->possibleScenario],

            [['date_of_birth', 'date_registered', 'date_of_application', 'skills', 'landmark', 'reasons', 'id_cards', 'documents', 'client_category', 'photo', 'family_composition', 'interests', 'work_experience', 'organizations', 'org_office_address', 'cause_of_disability', 'pwd_type_of_disability', 'is_senior'], 'safe'],
            ...
            [['sector_id', 'other_source_income', 'living_with_whom', 'encoded_by', 'edited_by', 'reason1', 'reason2', 'reason3', 'type_of_disability', 'org_tel_no'], 'string', 'max' => 64],
	...
	public function getExportColumns()
    {
        ...
        foreach ($columns as $key => &$column) {
            ...
            $column['label'] = strtoupper(str_replace('_', ' ', $column['attribute']));
    ...
    public function getMainInformationColumns()
    {
        switch ($this->priority_sector) {
            case self::SC_ID:
                $columns = [
                    'watershedName' => 'watershedName:raw',
                    'status' => 'status:raw',
    ...
    public function getAddress()
    {
        ...
        if ($this->house_no) {
            $address[] = 'House No. ' . $this->house_no;
        }
