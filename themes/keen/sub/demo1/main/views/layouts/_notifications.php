...
$this->registerJs(<<< JS
    const loadNotification = (stateNotification) => {
        $.ajax({
            ...
            success: function(s) {
                if(s.status == 'success') {
                    ...
                    loadNotification(s.stateNotification);
                }
                else {
                    loadNotification(stateNotification);
                }
            },
            error: function(e) {
                console.log(e);
    ...
    $('.notification-badge-menu').on('click', function() {
        ...
        $.ajax({
            url: app.baseUrl + 'notification/load-all',
            dataType: 'json',
            method: 'get',
            success: function(s) {
                $('.notification-badge-container').html(s.badge);
...
<div class="dropdown mr-1">
    <div class="topbar-item notification-badge-menu" data-toggle="dropdown" data-offset="10px,0px">
        ...
        <span class="notification-badge-container" style="cursor: pointer;">
            <?= Html::if($total > 0, function() use($total) {
