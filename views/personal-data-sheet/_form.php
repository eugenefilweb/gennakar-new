...
<div class="wizard wizard-2" id="kt_wizard" data-wizard-state="first" data-wizard-clickable="false">
	<div class="wizard-nav flex-lg-shrink-0 w-lg-300px w-xl-375px border-right px-8 py-12 px-lg-10">
		<div class="wizard-steps">
			<?= Html::foreach($step_forms, function($step_form, $key) use($step, $action, $model) {
				$class = ($step == $key)? 'current': (($step > $key)? 'completed': 'pending');

				$url = Html::ifElse($model->token, Url::to([$action, 'token' => $model->token, 'step' => $step_form['slug']]), Url::to([$action, 'step' => $step_form['slug']]));
				return <<< HTML
					<a href="{$url}">
		...
		<div class="row">
			<div class="col">
      			<div class="pb-5" data-wizard-type="step-content" data-wizard-state="current">
      				<?= $this->render('_form/' . (App::get('template') ?: $step_forms[$step]['slug']), [
      					'model' => $model,
      					'step' => $step,
						'action' => $action,
