...
<table border="1">
	<tbody>
		...
		<tr class="row-height">
			...
			<td><?= $model->getChildrenData(12) ?></td>
			<td><?= $model->getChildrenData(12, 'birthdate') ?></td>
		</tr>

		<?= App::foreach($model->extendedChildrens, fn ($children) => Html::tag('tr', implode('', [
			Html::tag('td', '', ['class' => 'bg-light-gray pl-7']),
			Html::tag('td', '', ['colspan' => '2']),
			Html::tag('td', $children['name'] ?? ''),
			Html::tag('td', App::formatter()->asDateFormat($children['birthdate'] ?? '', 'm/d/Y')),
		]), [
			'class' => 'row-height'
		])) ?>
	</tbody>
