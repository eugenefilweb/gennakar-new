        ...
        if ($this->template == 'per-barangay') {
            ...
            foreach ($this->models as $model) {
                ...
                foreach ($this->priority_sector as $row) {
                    $th .= Html::tag('th', $row['code']);
                    $active .= Html::tag('td', Html::number($model["{$row['id']}_active"]) ?: $this->default) ;

                    $male .=  Html::tag('td',  Html::number($model["{$row['id']}_active_male"]) ?: $this->default) ;

                    $female .=  Html::tag('td',  Html::number($model["{$row['id']}_active_female"]) ?: $this->default);
                ...
                $tbody .= <<< HTML
                    <tr>
                        <td rowspan="2"> {$model['barangay']} </td>
                        <td>Male</td>
                        {$male}
                    </tr>
