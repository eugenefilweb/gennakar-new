...
$js = <<< JS
    const filter = function(form, success) {
        KTApp.blockPage();
        $.ajax({
    ...
    $('.check-all-filter').on('change', function() {
        ...
        filter(form, function(s) {
            if(s.status == 'success') {
                ...
                for (var i = s.table_columns.length - 1; i >= 0; i--) {
                    var th = $('th[data-key="'+ s.table_columns[i] +'"]').show();
                    var td = $('td[data-key="'+ s.table_columns[i] +'"]').show();
                }
            }
            else {
...
<div data-widget_id="<?= $id ?>" class="dropdown dropdown-inline" data-toggle="tooltip" title="" data-placement="top" data-original-title="" style="<?= $style ?>"> 
        ...
        <div data-widget_id="<?= $id ?>" class="dropdown-menu dropdown-menu-sm dropdown-menu-right p-0 m-0" style="">
            ...
            <?= Html::beginForm(['user-meta/filter'], 'post',  [
                ...
                <ul class="navi navi-hover" style="padding: 10px;">
                    ...
                    <li class="navi-item">
                        <div class="checkbox-list">
                            <?= Html::foreach($searchModel->tableColumns, function($value, $key, $counter) use ($model, $filterColumns) {
                                return $counter == 3 ? Html::activeInput('hidden', $model, 'columns[]', ['value' => $key]): '<label class="checkbox">
    ...
    <?= App::if($searchModel, <<< HTML
        <a href="#!" class="btn btn-fixed-height btn-bg-white btn-text-dark-50 btn-hover-text-primary btn-icon-primary font-weight-bolder font-size-sm mt-1 mr-3 btn-sm"  style="border: 1px solid #ccc;" id="kt_quick_panel_toggle">
            <span class="svg-icon svg-icon-md">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
