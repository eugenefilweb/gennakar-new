...
$js = <<< JS
            ...
            html += '<div class="dd3-content">';
                html += '<div class="row">';
                    ...
                    html += '<span  style="position: absolute; right: 5px;">';
                        ...
                        html += '</a>';
                        html += '<a href="#!" class="btn btn-outline-secondary btn-sm btn-icon mr-2 btn-add-more-menu" title="Insert Below">';
    ...
    $(document).on('click', '.btn-add-more-menu', function(e) {
        e.preventDefault();

        const listItemContainer = $(this).closest('li.dd-item');

        index++;
     
        listItemContainer.after(generateListItem(index));
        $('#dd-{$id}').trigger('change');
    });

    $('#add-main-navigation-{$id}').on('click', function(e) {
        e.preventDefault();
        index++;
        $('#ol-dd-list-{$id}').prepend(generateListItem(index));
        $('#dd-{$id}').trigger('change');
        // initNestable();
    })
