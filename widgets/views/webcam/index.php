...
$this->registerCss(<<< CSS
	#webcam-container-{$widgetId} .btn-container {
		display: flex; 
...
$this->registerWidgetJs($widgetFunction, <<< JS
	let video = document.querySelector("#{$videoOptions['id']}"),
		...
		btnSwitchCamera = document.querySelector("#webcam-container-{$widgetId} .btn-switch-camera"),
	...
	let initCamera = async function() {
	    ...
	    try {
	    	stream = await navigator.mediaDevices.getUserMedia({ 
	    		video:  {width: {$videoOptions['width']}, height: {$videoOptions['height']}, facingMode}, 
	    		audio: false, 
    		});
...
<div id="webcam-container-<?= $widgetId ?>">
	...
	<div class="<?= $withNameInput ? 'input-group': '' ?> mt-3 webcam-input-group text-center">
		...
		<div class="btn-container">
			...
			<div>
				<?= Html::tag('a', 'Switch Camera', ['href' => '#', 'class' => 'btn btn-outline-secondary btn-switch-camera font-weight-bold']) ?>
			</div>
		</div>
	</div>
